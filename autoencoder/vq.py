...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, num_embed, embed_dim, beta, distance='cos',
        ...
        self.embedding.weight.data.uniform_(-1.0 /
                                            self.num_embed, 1.0 / self.num_embed)
    ...
    def forward(self, z, temp=None, rescale_logits=False, return_logits=False):
        ...
        if self.distance == 'l2':
            ...
            d = - torch.sum(z_flattened.detach() ** 2, dim=1, keepdim=True) - \
                ...
                2 * torch.einsum('bd, dn-> bn', z_flattened.detach(),
                                 rearrange(self.embedding.weight, 'n d-> d n'))
        elif self.distance == 'cos':
            # cosine distances from z to embeddings e_j
            normed_z_flattened = F.normalize(z_flattened, dim=1).detach()
            normed_codebook = F.normalize(self.embedding.weight, dim=1)
            d = torch.einsum('bd,dn->bn', normed_z_flattened,
        ...
        encodings = torch.zeros(encoding_indices.unsqueeze(
            1).shape[0], self.num_embed, device=z.device)
        ...
        if self.training:
            ...
            if self.anchor in ['closest', 'random', 'probrandom'] and (not self.init):
                ...
                # probabilitical based random sampling
                    ...
                    prob = torch.multinomial(
                        norm_distance, num_samples=1).view(-1)
                    random_feat = z_flattened.detach()[prob]
                # decay parameter based on the average usage
                decay = torch.exp(-(self.embed_prob*self.num_embed*10) /
                                  (1-self.decay)-1e-3).unsqueeze(1).repeat(1, self.embed_dim)
                self.embedding.weight.data = self.embedding.weight.data * \
                    (1 - decay) + random_feat * decay
                if self.first_batch:
                    self.init = True
            # contrastive loss
            if self.contras_loss:
                sort_distance, indices = d.sort(dim=0)
                dis_pos = sort_distance[-max(1, int(sort_distance.size(
                    0)/self.num_embed)):, :].mean(dim=0, keepdim=True)
                dis_neg = sort_distance[:int(sort_distance.size(0)*1/2), :]
                dis = torch.cat([dis_pos, dis_neg], dim=0).t() / 0.07
                contra_loss = F.cross_entropy(dis, torch.zeros(
                    (dis.size(0),), dtype=torch.long, device=dis.device))
                loss += contra_loss
